...
namespace RecordAndPlay.Editor.Extension
{
    ...
    /// </summary>
    [InitializeOnLoad]
    public class RecordingWindow : EditorWindow
    {
        ...
        [SerializeField]
        public bool startRecordingOnSceneStart = false;
        ...
        [SerializeField]
        public RecordingMethod currentRecordingMethod = RecordingMethod.Subjects;

        public static RecordingWindow Instance { get; private set; }

        /// <summary>
        /// This get's called *once* when the unity editor launches. Making 
        /// changes to this requires a full reload of the unity editor.
        /// </summary>
        ...
        private static void LogPlayModeState(PlayModeStateChange state)
        {
            ...
            if (window.recorder != null && window.recorder.CurrentlyStopped() == false)
            {
                RecordingUtil.SaveToAssets(window.recorder.Finish(), window.recordingName);
            }

            if (state == PlayModeStateChange.EnteredPlayMode && window.startRecordingOnSceneStart && string.IsNullOrEmpty(window.ReasonCantRecord()))
            {
        ...
        void OnGUI()
        {
            ...
            if (string.IsNullOrEmpty(ReasonCantRecord()))
            {
            ...
            if (EditorApplication.isPlaying == false)
            {
                startRecordingOnSceneStart = EditorGUILayout.Toggle("Record On Start", startRecordingOnSceneStart);
            }

            if (recorder == null || recorder.CurrentlyStopped())
            {
                var newRecordingName = EditorGUILayout.TextField("Recording Name", recordingName);
                if (string.Equals(newRecordingName, recordingName) == false)
                {
