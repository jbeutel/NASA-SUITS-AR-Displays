    ...
    public class SubjectRecorder
    {
        ...
        public void Capture(float time)
        {
            ...
            if (capturedPositions.Count > 1)
            {
                ...
                if ((time == lastPosition.Time && Vector3.Distance(pos, lastPosition.Vector) == 0) || V3Equal(lastPositionalVelocity, curPositionalVelocity))
                {
            ...
            if (capturedRotations.Count > 1)
            {
                ...
                if ((time == lastRotation.Time && Vector3.Distance(rot, lastRotation.Vector) == 0) || V3Equal(lastRotationalVelocity, curRotationalVelocity))
                {
